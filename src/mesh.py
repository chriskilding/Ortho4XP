...
if "dar" in sys.platform:
    ...
    triangle_cmd = os.path.join(filenames.Utils_dir, "triangle.app ")
    sort_mesh_cmd = os.path.join(filenames.Utils_dir, "moulinette.app ")
    unzip_cmd = "7z "
elif "win" in sys.platform:
    Triangle4XP_cmd = os.path.join(filenames.Utils_dir, "Triangle4XP.exe ")
    triangle_cmd = os.path.join(filenames.Utils_dir, "triangle.exe ")
    sort_mesh_cmd = os.path.join(filenames.Utils_dir, "moulinette.exe ")
    unzip_cmd = os.path.join(filenames.Utils_dir, "7z.exe ")
else:
    Triangle4XP_cmd = os.path.join(filenames.Utils_dir, "Triangle4XP ")
    triangle_cmd = os.path.join(filenames.Utils_dir, "triangle ")
    sort_mesh_cmd = os.path.join(filenames.Utils_dir, "moulinette ")
    unzip_cmd = "7z "


community_server = False
if os.path.exists(
    os.path.join(filenames.Ortho4XP_dir, "community_server.txt")
):
    try:
        f = open(
            os.path.join(filenames.Ortho4XP_dir, "community_server.txt"), "r"
        )
    ...
    try:
        ...
        if "[200]" in str(r):
            ...
            if subprocess.call(
                ...
                return 0
            os.remove(
                filenames.mesh_file(tile.build_dir, tile.lat, tile.lon) + ".7z"
            )
...
def build_curv_tol_weight_map(tile, weight_array):
    ...
    if tile.coast_curv_tol != tile.curvature_tol:
        ui.vprint(
            1,
            "-> Modifying curv_tol weight map according to coastline"
            " location.",
        )
        sea_layer = osm.OSM_layer()
        custom_coastline = filenames.custom_coastline(tile.lat, tile.lon)
        custom_coastline_dir = filenames.custom_coastline_dir(
            tile.lat, tile.lon
        )
        if os.path.isfile(custom_coastline):
            ...
            ui.vprint(
                1,
                "    * User defined custom coastline data detected (multiple"
                " files).",
            )
...
def post_process_nodes_altitudes(tile):
    ...
    f_node = open(filenames.output_node_file(tile), "r")
    ...
    f_ele = open(filenames.output_ele_file(tile), "r")
    ...
    f_node = open(filenames.output_node_file(tile), "w")
...
def write_mesh_file(tile, vertices):
    ...
    nbr_tri = int(f_ele.readline().split()[0])
    f = open(filenames.mesh_file(tile.build_dir, tile.lat, tile.lon), "w")
...
def extract_mesh_to_obj(
    ...
    obj_file_name = filenames.obj_file(
        til_x_left, til_y_top, zoomlevel, provider_code
    )
    mtl_file_name = filenames.mtl_file(
        til_x_left, til_y_top, zoomlevel, provider_code
    )
    ...
    f.write(
        "newmtl orthophoto\nmap_Kd "
        + filenames.geotiff_file_name_from_attributes(
            til_x_left, til_y_top, zoomlevel, provider_code
        )
        + "\n"
    )
...
def build_mesh(tile):
    ...
    ui.progress_bar(1, 0)
    poly_file = filenames.input_poly_file(tile)
    node_file = filenames.input_node_file(tile)
    alt_file = filenames.alt_file(tile)
    weight_file = filenames.weight_file(tile)
    ...
    Tri_option = (
        "-p" + do_refine + "uYB" + tri_verbosity + output_poly + limit_tris
    )
    ...
    curv_tol_scaling = sqrt(
        tile.dem.nxdem / (1000 * (tile.dem.x1 - tile.dem.x0))
    )
    hmin_effective = max(
        tile.hmin,
    ...
    if fingers_crossed.returncode:
        ui.vprint(
            ...
            + "It will be tempted now with no angle constraint (i.e."
            " min_angle=0).",
        )
        ...
        if fingers_crossed.returncode:
            ui.exit_message_and_bottom_line(
                ...
                + "If the reason is not due to the limited amount of RAM"
                " please\n"
    ...
    if ui.cleaning_level:
        try:
            os.remove(filenames.weight_file(tile))
        except:
            pass
        try:
            os.remove(filenames.output_node_file(tile))
        except:
            pass
        try:
            os.remove(filenames.output_ele_file(tile))
        except:
            pass
...
def sort_mesh(tile):
    ...
    ui.is_working = 1
    ui.red_flag = False
    mesh_file = filenames.mesh_file(tile.build_dir, tile.lat, tile.lon)
