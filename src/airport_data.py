                        ...
                        current_airport_data["runways"].append(
                            {
                                ...
                                "end_1_lat": numpy.float(
                                    m.group("runway_end_1_latitude")
                                ),
                                "end_1_lon": numpy.float(
                                    m.group("runway_end_1_longitude")
                                ),
                                "end_2_id": m.group("runway_end_2_number"),
                                "end_2_lat": numpy.float(
                                    m.group("runway_end_2_latitude")
                                ),
                                "end_2_lon": numpy.float(
                                    m.group("runway_end_2_longitude")
                                ),
...
class AirportDataSource:
    ...
    __PARSER_CLASSES__ = [
        XPlaneAptDatParser
    ]  # [OSMAirportsParser, XPlaneAptDatParser]
    ...
    def airports_in(cls, xp_tile, include_surrounding_tiles=False):
        ...
        # Load the airport data from the filesystem cache
        # (check if already loaded first)
        for t in filter(
            lambda x: x not in cls.__TILE_AIRPORTS__, search_range
        ):
    ...
    def _read_cached_tile(cls, tile):
        ...
        # Ensure the tile cache file has been built :
        # first waits for the main job to parse the apt.dat.
        # We need that because the tile updater jobs won't be
        # started until that part is done.
        if (
        ...
        # If we don't have an updater job for this tile,
        # then it means there wasn't any airport data for it,
    ...
    def _parse_apt_dat(_apt_dat_file):
        ...
        for _arpt in apt_dat_parser.parse(_apt_dat_file):
            for _rw in _arpt.values():
                for _t in _rw.relevant_xp_tiles(
                    include_surrounding_tiles=False
                ):
    ...
    def wait_for_cache_update(cls):
        ...
        if cls.cache_update_in_progress():
            concurrent.futures.wait(
                list(cls._cache_updater_tile_jobs.values())
            )
    ...
    def cache_update_in_progress(cls):
        return (
            ...
            cls._cache_updater_tile_jobs is not None
            and any(
                [j.running() for j in cls._cache_updater_tile_jobs.values()]
            )
        )
