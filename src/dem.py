...
class DEM:
    ...
    def create_normal_map(self, pixx, pixy):
        ...
        dx[:, 1:-1] = (self.alt_dem[:, 2:] - self.alt_dem[:, 0:-2]) / (
            2 * pixx
        )
    ...
    def super_level_set(self, level, wgs84_bbox):
        ...
        if ymax > self.y1:
            ymax = self.y1
        pixx0 = round(
            (xmin - self.x0) / (self.x1 - self.x0) * (self.nxdem - 1)
        )
        pixx1 = round(
            (xmax - self.x0) / (self.x1 - self.x0) * (self.nxdem - 1)
        )
        pixy0 = round(
            (self.y1 - ymax) / (self.y1 - self.y0) * (self.nydem - 1)
        )
        pixy1 = round(
            (self.y1 - ymin) / (self.y1 - self.y0) * (self.nydem - 1)
        )
        return (
            (
                xmin + self.lon,
...
def ensure_elevation(source, lat, lon, verbose=True):
    if source == "View":
        ...
        url_base = (
            "https://prd-tnm.s3.amazonaws.com/StagedProducts/Elevation/13/IMG/"
        )
        usgs_name = (
            "USGS_NED_13_n" + str(lat + 1) + "w" + str(-lon).zfill(3) + "_IMG"
        )
        ...
        r = http_request(
            url_base + "USGS_NED_1_" + usgs_base + "_ArcGrid.zip",
            source,
            verbose,
        )
        if not r:
